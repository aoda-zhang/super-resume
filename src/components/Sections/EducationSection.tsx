  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-slate-700">教育背景</h3>
        <button
          onClick={addItem}
            ...
            {expandedId === item.id && (
              <div className="p-4 space-y-4 bg-white">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm text-slate-600 mb-1">学校</label>
                    <input type="text" value={item.school} onChange={(e) => updateItem(item.id, 'school', e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg" placeholder="学校名称" />
                  </div>
                  <div>
                    <label className="block text-sm text-slate-600 mb-1">学位</label>
